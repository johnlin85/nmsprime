...
function ss_docsis($hostname, $snmp_community = null)
{
    ...
    if ($preEquData = ss_docsis_get_pnm()) {
...
function ss_docsis_get_pnm()
{
    ...
    if (preg_match_all("/^([^\.].+?):(.+)/m", $file, $match) != count(array_flatten($required)) ||
        array_diff(array_flatten($required), $match[1])) {
        return [];
    }

    $preEquData = array_combine($match[1], $match[2]);
    foreach ($required['explode'] as $explode) {
        $preEquData[$explode] = explode(',', $preEquData[$explode]);
    }
